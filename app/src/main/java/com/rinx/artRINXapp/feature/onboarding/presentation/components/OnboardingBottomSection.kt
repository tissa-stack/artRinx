...
fun OnboardingControls(
    ...
    Row(
        ...
        // Progress pills: exactly [pageCount] fixed-size pills that fill cumulatively —
        // every completed pill (up to & including the current page) takes the active
        // colour; upcoming pills stay secondary. The active pill also scales 1.0 → 1.1.
        Row(
        ...
        // Next button: a circular progress ring that fills as pages advance, with an
        // inner #45B1E8 circle + white chevron. Ring fill uses the page spring.
        val arcProgress by animateFloatAsState(
            targetValue = (currentPage + 1).toFloat() / pageCount.toFloat(),
            animationSpec = OnboardingAnim.pageFloatSpec(),
            label = "arc_progress",
        )
